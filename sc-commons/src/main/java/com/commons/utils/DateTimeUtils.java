...
public final class DateTimeUtils {
     ...
     * 年月日小时(yyyyMMddHH)
     */
    public static final String YEAR_MONTH_DAY_TO_HOUR = "yyyyMMddHH";
    ...
    public static Date addMin(Date date, long min) {
        ...
        Date afterDate = new Date(date.getTime() + min);
        return afterDate;
    }
    ...
    /**
     * @Description(功能描述): 20180820100500转2018-08-20 10:05:00
     * @author(作者): panpengfei
     * @date(开发日期): 2018/8/20 10:05
    **/
    public static String getDateString(String dateString){
        try{
            SimpleDateFormat sdfYear = new SimpleDateFormat("yyyyMMddHHmmss");
            Date date = sdfYear.parse(dateString);
    ...
    /**
     * @Description(功能描述): 20180820100500转2018年08月20日 10时05分
     * @author(作者): panpengfei
     * @date(开发日期): 2018/8/20 10:05
     **/
    public static String getDateFormatString(String dateString){
        try{
            SimpleDateFormat sdfYear = new SimpleDateFormat("yyyyMMddHHmm");
            Date date = sdfYear.parse(dateString);
            SimpleDateFormat sdfDate = new SimpleDateFormat("yyyy年MM月dd日 HH时mm分");
            String time = sdfDate.format(date);
            return time;
        }catch (Exception e){
            e.printStackTrace();
    ...
    public String getApprovalTime(String dateStartTime,String dateEndTime){
        Date dateVar0 = DateTimeUtils.formatDateString(dateStartTime,"yyyyMMddHHmmss");
        Date dateVar1 = DateTimeUtils.formatDateString(dateEndTime,"yyyyMMddHHmmss");

        long diff = dateVar0.getTime() - dateVar1.getTime();//这样得到的差值是微秒级别
        Calendar currentTimes =dataToCalendar(dateVar0);//当前系统时间转Calendar类型
        Calendar  firstTimes =dataToCalendar(dateVar1);//查询的数据时间转Calendar类型
    ...
    public static String getCurrentDateTime(String format) {
        return LocalDateTime.now().format(java.time.format.DateTimeFormatter.ofPattern(format));
    }
    public static String getCurrentDateTime2() {
        return LocalDateTime.now().format(java.time.format.DateTimeFormatter.ofPattern(YEAR_MONTH_DAY_HOUR_MINUTE_SECOND));
    }

    public static String getCurrentDateTime3() {
        return LocalDateTime.now().format(java.time.format.DateTimeFormatter.ofPattern(YEAR_MONTH_DAY_HOUR_MINUTE_SECOND_SIMPLE));
    }
    public static String getCurrentDateTime4() {
        return LocalDateTime.now().format(java.time.format.DateTimeFormatter.ofPattern(YEAR_MONTH_DAY_SIMPLE));
    }
    public static String getCurrentDateTimeHour() {
        return LocalDateTime.now().format(java.time.format.DateTimeFormatter.ofPattern(YEAR_MONTH_DAY_TO_HOUR));
    }

    public static String getCurrentDateTime5() {
        return LocalDateTime.now().format(java.time.format.DateTimeFormatter.ofPattern(YEAR_MONTH_DAY));
    }

    //20170202020202 转 date
    public static Date dateStringToDate_2(String dateString){
        if(org.apache.commons.lang3.StringUtils.isEmpty(dateString)){
            return null;
        }

        try{
            return ymdhms.parse(dateString);
        }catch (Exception e){
            e.printStackTrace();
            return null;
        }
    ...
    public static String dateToString(Date date){
        if ( date != null) {
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat(YEAR_MONTH_DAY_HOUR_MINUTE_SECOND_SIMPLE);
    ...
    public static Map getDateMonth(){
        // 获取当月第一天和最后一天
        ...
        Map map = new HashMap();
        map.put("startTime",firstday + "000000");
        map.put("endTime",lastday + "000000");
        return map;
    }


    public static void main(String[] args)throws Exception{
//        System.out.println(getDays("2018-01-01","2018-03-03"));
        System.out.println(getCurrentDateTime3());

//        System.out.println(getBetweenDates(DateUtil.dateStringDToDate("2018-01-01"),DateUtil.dateStringDToDate("2018-03-03")));
    }
